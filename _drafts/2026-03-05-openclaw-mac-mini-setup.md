...
이 문서는 설치 절차만 다룬다. 후기/회고는 별도 문서에 작성한다.
...
Chrome Remote Desktop는 구글 계정 기반으로 브라우저에서 원격 화면을 붙는 도구다.
...
## 3-2단계. VNC 접속 설정
VNC(Virtual Network Computing)는 macOS 기본 화면 공유 기능으로 접속하는 방식이다.
